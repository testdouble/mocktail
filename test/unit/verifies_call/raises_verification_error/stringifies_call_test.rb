...
module Mocktail
  class StringifiesCallTest < Minitest::Test
    ...
    def test_some_calls
      ...
      # Blocks & Procs
      assert_equal "hi { Proc at test/unit/verifies_call/raises_verification_error/stringifies_call_test.rb:58 }", invoke {}
      assert_equal "hi(&lambda[test/unit/verifies_call/raises_verification_error/stringifies_call_test.rb:59])", invoke(&lambda {})

      # Mix & Match
      assert_equal "hi(:a, 1, b: 2) { Proc at test/unit/verifies_call/raises_verification_error/stringifies_call_test.rb:62 }", invoke(args: [:a, 1], kwargs: {b: 2}) { |c| 3 }
    end
